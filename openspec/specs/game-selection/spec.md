...
Activating a game card SHALL navigate to that game's route without a full page reload. Each game route SHALL render a page scoped to that game; until game-specific features exist, a placeholder page showing the game's name satisfies this.

#### Scenario: Navigate to Weiss Schwarz

- **WHEN** the user activates the Weiss Schwarz card
- **THEN** the app navigates to `/weiss-schwarz` and shows the Weiss Schwarz page

#### Scenario: Navigate to Gundam Card Game

- **WHEN** the user activates the Gundam Card Game card
...
- **THEN** the corresponding game page renders (SPA rewrite serves the app on any path)
